...
function showProducts(products) {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    card.addEventListener("click", () => {
      ...
      document.getElementById("sidebar-description").innerText =
        product.description;
...
function addToCard() {
  ...
  if (isExisting) {
    ...
    existingProduct.quantity =
      Number(existingProduct.quantity) + Number(qty.value);
...
function runProductSearch() {
  ...
  for (let i = 0; i < productCards.length; i++) {
    ...
    let title = card.querySelector(".product-name")?.innerText || "";
    let category = card.querySelector(".product-category")?.innerText || "";
    let cardText = `${title} ${category}`.toLowerCase();
    let column = card.closest(".col-md-3, .col-xs-6");
...
let searchInputElem = document.querySelector("input[placeholder*='Search']");
if (searchInputElem) {
  searchInputElem.addEventListener("keyup", runProductSearch);
